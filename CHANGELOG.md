...
## 0.11.4 (2016-04-16)
* Added Gulp support (although if you're not using CodeKit, you really are doing it wrong). To use Gulp with LUMEN, switch to the theme directory and run `npm install`, then `npm start`
...
* Added phpdocs for the core LUMEN library; located in the theme's `_docs` folder.

## 0.11.0 (2016-02-27)
* Updated LUMEN to use Foundation for Sites 6.2!
* LUMEN now uses PSR-4 class autoloading. No more manual requires!
...
* Applied WordPress coding standards to core LUMEN library.
...
## 0.10.0 (2016-01-22)
* Updated LUMEN to use Foundation 6
...
* Foundation's humans.txt and robots.txt are no longer included in LUMEN Theme Framework
...
* LUMEN now uses the compiler-agnostic version of Foundation, which means compiling with libsass is now an option
* For more info on why Compass was removed, see <http://www.lumenframework.org/foundation-5-4-5-sass-compatibility-alert/>
...
* Related: A preliminary version of LumenFramework.org is now live (but still needs so, so much work)
